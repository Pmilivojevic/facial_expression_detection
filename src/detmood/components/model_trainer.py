...
class ModelTrainer:
    ...
    This class handles model initialization, training, validation, saving the best-performing
    models, and plotting training/validation metrics.
    
    Attributes:
        config (ModelTrainerConfig): Configuration object with training parameters, file paths,
                                     and model parameters.
        dataset (CustomImageDataset): The dataset to be used for model training and validation.
        splits (Iterator): The cross-validation splits generated by StratifiedKFold or similar
                            methods, containing train and validation indices.
    """
    
    def __init__(self, config: ModelTrainerConfig, dataset: CustomImageDataset, splits: Iterator):
        """
        Initializes the ModelTrainer with configuration settings, dataset, and data splits for
        training.

        Args:
            config (ModelTrainerConfig): Configuration object that contains model parameters, 
                                         dataset information, and training settings.
            dataset (CustomImageDataset): The dataset to be used for model training and validation.
            splits (Iterator): The cross-validation splits generated by StratifiedKFold or similar
                               methods, containing train and validation indices.
        """
    ...
    def train(self):
        ...
        for fold, (train_idx, val_idx) in tqdm(enumerate(self.splits)):
